...
function cacheElements() {
  refs = {
    ...
    codeSnippetLines: document.getElementById("code-snippet-line-numbers"),
    ...
    resultCategoryBreakdown: document.getElementById("result-category-breakdown"),
...
function bindEvents() {
  ...
  refs.copyCodeBtn?.addEventListener("click", handleCopyCodeSnippet);
...
function renderQuestion() {
  ...
  updateLevelChipStyle(levelObj.level);
...
function renderCodeSnippet(questionObj) {
  if (!refs.codeSnippetBox || !refs.codeSnippetText || !refs.codeSnippetLines) {
    return;
  }
  ...
  if (!snippet.trim()) {
    ...
    refs.codeSnippetLines.textContent = "";
  ...
  refs.codeSnippetLines.textContent = lines.map((_, index) => String(index + 1)).join("\n");
  refs.codeSnippetText.innerHTML = highlightCodeSnippet(snippet);

  if (refs.copyCodeBtn) {
    refs.copyCodeBtn.textContent = "Copy";
    refs.copyCodeBtn.disabled = false;
    refs.copyCodeBtn.dataset.code = snippet;
  }
...
function updateLevelChipStyle(levelName) {
  if (!refs.levelChip) {
    return;
  }

  refs.levelChip.classList.remove("chip-level-basic", "chip-level-intermediate", "chip-level-advanced");

  const normalized = String(levelName || "").toLowerCase();
  if (normalized === "basic") {
    refs.levelChip.classList.add("chip-level-basic");
  } else if (normalized === "intermediate") {
    refs.levelChip.classList.add("chip-level-intermediate");
  } else if (normalized === "advanced") {
    refs.levelChip.classList.add("chip-level-advanced");
  }
}

async function handleCopyCodeSnippet() {
  if (!refs.copyCodeBtn) {
    return;
  }

  const code = refs.copyCodeBtn.dataset.code || "";
  if (!code.trim()) {
    return;
  }
  ...
  try {
    ...
    refs.copyCodeBtn.textContent = "Failed";
    window.setTimeout(() => {
      if (refs.copyCodeBtn) {
        refs.copyCodeBtn.textContent = "Copy";
      }
...
function highlightCodeSnippet(snippet) {
  ...
  const keywordRegex = /\b(var|let|const|if|else|for|while|return|function|new|try|catch|finally|switch|case|break|continue)\b/g;
  const stringRegex = /(\"[^\"\\]*(\\.[^\"\\]*)*\"|'[^'\\]*(\\.[^'\\]*)*')/g;
  const numberRegex = /\b\d+(\.\d+)?\b/g;
  const commentRegex = /(\/\/[^\n]*)/g;

  return escaped
    .replace(commentRegex, '<span class="code-token-comment">$1</span>')
    .replace(stringRegex, '<span class="code-token-string">$1</span>')
    .replace(keywordRegex, '<span class="code-token-keyword">$1</span>')
    .replace(numberRegex, '<span class="code-token-number">$&</span>');
}
...
function renderResult() {
  ...
  const wrongCount = Math.max(0, total - quizState.score);
  ...
  if (refs.resultWrongCount) {
    refs.resultWrongCount.textContent = String(wrongCount);
  }
...
function calculateCategoryBreakdown() {
  ...
  quizData.levels.forEach((levelObj, levelIndex) => {
    if (!Array.isArray(levelObj?.questions)) {
      return;
    }

    levelObj.questions.forEach((questionObj, questionIndex) => {
      const key = String(questionObj?.category || "general").trim().toLowerCase() || "general";
      if (!breakdown[key]) {
...
function renderCategoryBreakdown(breakdown) {
  if (!refs.resultCategoryBreakdown) {
    return;
  }

  refs.resultCategoryBreakdown.innerHTML = "";

  const entries = Object.entries(breakdown || {}).sort((a, b) => a[0].localeCompare(b[0]));
  if (entries.length === 0) {
    refs.resultCategoryBreakdown.innerHTML = '<p class="result-label">No category data available.</p>';
    return;
  }

  entries.forEach(([category, stats]) => {
    ...
    value.textContent = `${stats.correct}/${stats.total} correct (${wrong} wrong)`;

    row.appendChild(label);
    row.appendChild(value);
    refs.resultCategoryBreakdown.appendChild(row);
  });
}
